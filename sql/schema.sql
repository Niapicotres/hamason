create table cart (customer_id bigint not null unique, id bigint generated by default as identity, primary key (id));
...
create table customer (id bigint generated by default as identity, address varchar(255) not null, creditCardExpiry varchar(255) not null, creditCardNumber varchar(255) not null, email varchar(255) not null unique, phone varchar(255) not null, user_id varchar(255) not null unique, primary key (id));
...
create table orders (creationDate date not null, customer_id bigint not null, id bigint generated by default as identity, primary key (id));
...
alter table if exists cart add constraint FKdebwvad6pp1ekiqy5jtixqbaj foreign key (customer_id) references customer;
...
alter table if exists customer add constraint FK5k70oa5i1gx1p1kojh2meejvu foreign key (user_id) references USERS;
